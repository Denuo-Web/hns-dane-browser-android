...
- Current iOS release candidate: `0.5.0` (`43`)
...
4. verifies the archived app identity and compiled AppIcon catalog, then validates/exports the archive with App Store Connect authentication and uploads build `43`;
5. deletes the temporary keychain, installed profile, API key, `.p12`, and profile while GitHub discards the runner.

Apple associates the uploaded build with the app record using its bundle ID, version, and build number. A rerun after Apple accepts build `43` requires a higher build number.

Build `43` declares `ITSAppUsesNonExemptEncryption = false` because the first
