...
| Build supply chain | Local and hosted gates pass; not continuously enforced | Local Rust, dependency, Android unit, lint, and signed-bundle checks passed. The `0.4.1` code and build-policy tree passed the hosted path-policy, Rust, cold-cache Android, Xcode 26.5 Apple, and required-result jobs in [GitHub Actions run 29477163745](https://github.com/Denuo-Web/hns-dane-browser/actions/runs/29477163745); only this evidence-only documentation update followed. Actions was then restored to the repository's prior disabled state, and `main` still has no protection or ruleset. |
