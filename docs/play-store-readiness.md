...
This checklist maps HNS DANE Browser to current Google Play update requirements and identifies the Play Console fields that must be reconciled outside the repository. The app is already public: the live production listing observed during this audit serves `0.3.1` (`versionCode 22`). The repository release candidate declares `0.3.14` (`versionCode 35`).
...
| Android App Bundle | Ready locally | Package identity is `com.denuoweb.hnsdane`. The final `0.3.14` upload-signed AAB passed the structural, native-hardening, and entry-by-entry signer gates. |
...
2. Build and verify `dist/play-store/hns-dane-browser-v0.3.14-play-upload-signed.aab` with the exact release toolchain; the automated gate covers 16 KiB alignment, required ABIs, native hardening/symbols, R8 mapping, notices, and upload signing.
